...
- **A group chat is never a household.** `TelegramRoutingService` splits group/supergroup updates off before any
  ...
  `conversation_state`. The bot reacts in a group only to a reply to its own message or a tap on its own button — a mention, a
  command or plain chatter is dropped at DEBUG and never reaches a model. Every count in that flow
